...
# import logging
# import uuid
# from flask import Flask, render_template, request, send_file, url_for, send_from_directory, session, redirect
...
# def process_image(file_obj, stored_filename):
#     if file_obj and file_obj.filename:
#         return save_uploaded_image(file_obj)
...
#                         new_para.add_run().add_picture(img_path, width=Inches(float(sec.get("image_width", "2"))))
#                         new_para.add_run("    ")
...
#             # Par défaut, en GET, la valeur sélectionnée est "insertion_default"
...
#         try:
#             sections = json.loads(request.form.get("formData", "[]"))
#         except:
#             sections = []

#         const_text = request.form.getlist("text_content[]")
#         const_font = request.form.getlist("font_size[]")
#         const_align = request.form.getlist("alignment[]")
#         const_highlight = request.form.getlist("highlight[]")
#         const_bold = request.form.getlist("bold[]")
#         const_italic = request.form.getlist("italic[]")
#         const_underline = request.form.getlist("underline[]")
#         const_color = request.form.getlist("text_color[]")
#         const_image_width = request.form.getlist("image_width[]")
#         const_original = request.form.getlist("original[]")
#         const_hl_color = request.form.getlist("highlight_color[]")
...
#             try:
#                 insertion_index = int(insertion_index_field)
#             except:
#                 insertion_index = insertion_default if insertion_default is not None else 0
#         else:
#             insertion_index = insertion_default if insertion_default is not None else 0

#         # Conserver la sélection dans le menu après soumission
...
#                 for sec in sections:
#                     if sec.get("type") == "text":
#                         doc.add_paragraph(sec.get("text_content", ""))
#                     elif sec.get("type") == "images":
...
#                                 try:
#                                     img_path = os.path.join(UPLOAD_FOLDER, img_filename)
#                                     p_img.add_run().add_picture(img_path, width=Inches(float(sec.get("image_width", "2"))))
#                                     p_img.add_run("    ")
#                                 except Exception as e:
#                                     logging.error("Erreur lors de l'insertion d'image: %s", e)
#             else:
                        ...
                        for key in ["stored_image1", "stored_image2"]:
                            ...
                            if img_filename:
                                try:
                                    image_data = in_memory_images.get(img_filename)
                                    if image_data:
                                        p_img.add_run().add_picture(BytesIO(image_data), width=Inches(float(sec.get("image_width", "2"))))
                                        p_img.add_run("    ")
                                    else:
                                        logging.error("Image data not found for %s", img_filename)
                                except Exception as e:
